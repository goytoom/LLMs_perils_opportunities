...
```{r}
df <- read.csv(original_path) # load predictions
df <- df %>% rename(pred_care = care, pred_equality = equality, pred_loyalty = loyalty, 
                    pred_authority = authority,
                  pred_purity = purity, pred_proportionality = proportionality, 
                  pred_thin = thin.morality, pred_non = non.moral)
...
df <- merge(df, meta, by = "annotator") #merge dataset with annotator information
```
...
## Models
Train linear models for each type of bias
```{r}
# demographics
...
## Evaluate
Show summaries for each type of bias and extract coefficients
```{r}
summary(lm_demo)
...
results_path_3 = paste(evals_dir, paste(corp, "success", type, mode, sep="_"), "_liwc.csv", sep="") #liwc
results_path_4 = paste(evals_dir, paste("gpt_fewshot", corp, "success", mode, sep="_"), ".csv", sep="") #fewshot
results_path_5 = paste(evals_dir, paste("gpt_FT", corp, "success", mode, sep="_"), ".csv", sep="") #finetuned
results_path_gt = paste("../data/preprocessed/", paste(corp, "sample", mode, sep="_"), ".csv", sep = "") # humans
...
df4 <- read.csv(results_path_4) %>% select(-success, -annotator, -annotation) %>% distinct(.keep_all = TRUE)# load predictions
df5 <- read.csv(results_path_5) %>% select(-success, -annotator, -annotation) %>% distinct(.keep_all = TRUE)# load predictions
...
df2$group = "chatGPT"
df3$group = "LIWC"
df4$group = "chatGPT-FEW"
df5$group = "chatGPT-FT"
df_groundtruth$group = "TRUE"

df_total = rbind(df1,df2, df3, df4, df5, df_groundtruth)
df_total$group <- relevel(as.factor(df_total$group), ref = "TRUE")
```

## Fit Model (regress predicted moral sentiment over type of model/humans)
```{r}
...
for(model in list_models){
  ...
  coef_gpt = (round(exp(tidy(model)$estimate[3]), 2)-1)*100
  coef_gpt_few = (round(exp(tidy(model)$estimate[4]), 2)-1)*100
  coef_gpt_ft = (round(exp(tidy(model)$estimate[5]), 2)-1)*100
  coef_liwc = (round(exp(tidy(model)$estimate[6]), 2)-1)*100
  coefList[[length(coefList)+1]] = list(c(coef_bert, coef_liwc, coef_gpt, coef_gpt_few, coef_gpt_ft))
  i = i + 1
}

df_coefs = as.data.frame(matrix(unlist(coefList), nrow=length(unlist(coefList[1]))))
colnames(df_coefs) = list_names
model_names <- c("BERT", "zeroshot-GPT", "fewshot-GPT", "finetuned-GPT", "LIWC")
df_coefs <- cbind("model name" = model_names, df_coefs)

# order: bert, zeroshot-gpt, fewshot-gpt, finetuned-gpt, liwc
df_coefs
```

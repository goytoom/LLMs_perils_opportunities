...
data = "closure" # change to respective survey you are investigating
...
groupComparisons <- function(df, varnames){
  ...
  for(varname in varnames){
    ...
    df_desc$group <- factor(df_desc$group, levels = c('LLAMA', "GPT", unique(human_cnstr$group)))
    ...
    if(length(constructs)>1){
      # llama_cnstr_avg = colMeans(llama_cnstr[, !(names(llama_cnstr) %in% c("group"))], na.rm = TRUE)
      ...
      # llama_cnstr_avg = mean(llama_cnstr[, !(names(llama_cnstr) %in% c("group"))], na.rm = TRUE)
    ...
    # diff_avg[,names(constructs)] = sweep(human_cnstr_avg[,names(constructs)], 2, llama_cnstr_avg)
...
#save plot
ggsave(filename = paste("../results/plots/llama/", data, "_bias_estimates_llama.png", sep=""), est, width = 8, height = 4, dpi = 150, units = "in", device='png')
```

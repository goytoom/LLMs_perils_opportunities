...
```{r}
rm(list = ls()) #remove all variables before analysis
...
llama_path = paste(evals_dir, data, "_llama2.csv", sep="") #llama
gpt_path = paste(evals_dir, data, ".csv", sep="") #gpt3.5
survey_path = paste(survey_dir, data, "_cleaned", ".csv", sep="") #humans

df_llama <- read.csv(llama_path) # load predictions
df_gpt <- read.csv(gpt_path) # load predictions
df_human <- read.csv(survey_path) # load human responses
print(nrow(df_human))
      
n_items = nrow(df_llama)
...
```{r warning=F}
####### LLAMA
ncols = ncol(df_llama) # transform to numeric
df_llama2 <- df_llama %>% mutate(across(starts_with("response_"), ~ as.numeric(gsub("([0-9]+).*$", "\\1", .))))

if(length(reverse_code)!=0){
df_llama2[reverse_code, 3:ncols] = apply(df_llama2[reverse_code, 3:ncols], 2, item.reverse, min=min_val, max=max_val)
}
df_llama2 = newCols(df_llama2, constructs, "llama") # create construct scores

# get average responses across all repeats
llama_avg <- rowMeans(df_llama2[, 3:ncols], na.rm = TRUE)
print(sum(is.na(df_llama2)))
...
df_gpt2 = newCols(df_gpt2, constructs, "llama") # create construct scores
...
# get difference of humans and llama on items
df_diff = df_humans_r
df_diff[, 1:(length(llama_avg) - length(constructs))] = abs(df_diff[, 1:(length(llama_avg) - length(constructs))] - llama_avg[1:(length(llama_avg)-5)])

# get difference of humans and llama on constructs
df_diff[, (ncol(df_diff)-length(constructs)+1):ncol(df_diff)] = abs(df_diff[, (ncol(df_diff)-length(constructs)+1):ncol(df_diff)] - llama_avg[(length(llama_avg)-length(constructs)+1):length(llama_avg)])
...
```{r}
color_mapping <- list(age = c("LLAMA" = "#ffff33", "GPT" = "#e41a1c", "18-24" = "#377eb8", "25-34" = "#4daf4a", "35-54" = "#984ea3", "55-74" = "#ff7f00"),
                      political_opinion = c("LLAMA" = "#ffff33","GPT" = "#e41a1c", "Conservative" = "#377eb8", "Liberal" = "#4daf4a", "Moderate" = "#984ea3"),
                      race = c("LLAMA" = "#ffff33","GPT" = "#e41a1c", "Asian" = "#377eb8", "Black" = "#4daf4a", "Hispanic" = "#984ea3", "White" = "#ff7f00"),
                      religion = c("LLAMA" = "#ffff33","GPT" = "#e41a1c", "Agnostic" = "#377eb8", "Atheist" = "#4daf4a", "Christianity" = "#984ea3"),
                      sex = c("LLAMA" = "#ffff33", "GPT" = "#e41a1c", "Male" = "#377eb8", "Female" = "#4daf4a")
                      )
...
groupComparisons <- function(df, varnames){
  ...
  for(varname in varnames){
    ...
    llama_cnstr["group"] = "LLAMA"
    gpt_cnstr["group"] = "GPT"
    df_desc = rbind(human_cnstr, llama_cnstr, gpt_cnstr)
    df_desc$group <- factor(df_desc$group, levels = c('LLAMA', "GPT", sort(unique(human_cnstr$group))))
    
    print(sort(unique(human_cnstr$group)))
    
    human_cnstr_sd = human_cnstr %>% group_by(group) %>% summarise(across(everything(), \(x) sd(x, na.rm = TRUE)))
    
    if(length(constructs)>1){
      llama_cnstr_sd = sapply(llama_cnstr[, !(names(llama_cnstr) %in% c("group"))], sd, na.rm = TRUE)
      gpt_cnstr_sd = sapply(gpt_cnstr[, !(names(gpt_cnstr) %in% c("group"))], sd, na.rm = TRUE)
    }else{
      llama_cnstr_sd = sd(llama_cnstr[, !(names(llama_cnstr) %in% c("group"))], na.rm = TRUE)
      gpt_cnstr_sd = sd(gpt_cnstr[, !(names(gpt_cnstr) %in% c("group"))], na.rm = TRUE)
    }
    
    #Dunnet, are group means different from LLaMa?
    if(nrow(human_cnstr)>0){
    for(col in colnames(df_desc)){
      if(col != "group"){
        ...
        # print(summary(model))
        print(col)
        print(DunnettTest(f, data = df_desc, control = "LLAMA"))
      }
    }
    print("Bonferroni corrected p-value:")
    print(0.05/length(constructs))
    
    # calculate difference of variance for LLama vs demographic groups and GPT
    diff_sd = human_cnstr_sd
    diff_sd[,names(constructs)] = sweep(human_cnstr_sd[,names(constructs)], 2, llama_cnstr_sd)
    diff_sd_models = gpt_cnstr_sd - llama_cnstr_sd
    diff_sd_models = c("GPT", diff_sd_models)
    names(diff_sd_models) <- c("group", names(constructs))
    diff_sd = rbind(diff_sd, diff_sd_models)
    
  
    # test differences of variance on all constructs (p values)
    for(g in unique(df_desc$group)){
      if(g!="LLAMA"){
        df1 = df_desc %>% filter(group==g | group=="LLAMA") %>% select(names(constructs))
        df2 = df_desc %>% filter(group==g | group=="LLAMA")
        print(g)
    ...
    t1 <- ggplot(df_comparison, aes(x = construct, y = value, fill = group)) +
      PupillometryR::geom_flat_violin(aes(fill = group), scale = "width", position = position_nudge(x = .2, y = 0), adjust = 1.5, trim = FALSE, alpha = .5, colour = NA)+
      geom_point(aes(x = construct, y = value, colour = group), position = ggpp::position_jitternudge(width=0.05, height=0.075, x = -.25, nudge.from = "jittered"), size = 1, shape = 20, alpha = 0.65)+
      geom_boxplot(aes(x = construct, y = value, fill = group),outlier.shape = NA, lwd=.1, alpha = .65, width = .35, colour = "black")+
      scale_colour_manual(values = color_mapping[[varname]]) + xlab("") + ylab("Construct Score") + theme_apa(y.font.size=11) + scale_fill_manual(values = color_mapping[[varname]]) # + scale_fill_brewer(palette = "Set1") + scale_colour_brewer(palette = "Set1")
    
    print(t1)
    
    ggsave(filename = paste("../results/plots/llama/", data, "_", varname, "_llama.png", sep=""), t1, width = 8, height = 4, dpi = 150, units = "in", device='png')
    
    print("---------------------------------------------------")
    }else{
    print("Not enough data for any demographic group")
    }
  }
...
llama_cnstr = transpose(df_llama2[(length(llama_avg)-length(constructs)+1):length(llama_avg),3:ncols])
colnames(llama_cnstr) <- names(constructs)
